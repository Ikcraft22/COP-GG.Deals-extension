...
function resolveGamePressureTitle(): string | null {
...
function normalizeGamePressureExternalUrl(value: string | null): string | null {
    if (!value) {
        return null;
    }
...
export function resolveGamePressureRequestUrls(): string[] {
    const links = document.querySelectorAll<HTMLAnchorElement>(
        'a[href*="store.steampowered.com/app/"], a[href*="opencritic.com/game/"]',
    );
    const requestUrls = Array.from(links)
        .map((link) => normalizeGamePressureExternalUrl(link.getAttribute('href')))
        .filter((requestUrl): requestUrl is string => requestUrl !== null);

    return [...new Set(requestUrls)];
}

export const gamePressurePageRule: DomainPageRule = {
    domain: 'gamepressure.com',
    resolveTitle: resolveGamePressureTitle,
    resolveRequestUrls: resolveGamePressureRequestUrls,
